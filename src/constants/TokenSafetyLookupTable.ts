...
import { UNI_EXTENDED_LIST, UNI_LIST, UNSUPPORTED_LIST_URLS, HARMONY_TOKEN_LIST } from './lists'
...
class TokenSafetyLookupTable {
  ...
  createMap() {
    ...
    // add import HARMONY List
    store.getState().lists.byUrl[HARMONY_TOKEN_LIST].current?.tokens.forEach((token) => {
