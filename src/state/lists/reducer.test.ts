      ...
      beforeEach(() => {
        store = createStore(reducer, {
          byUrl: {
            ...
            'https://unpkg.com/@thinkincoin-libs/default-token-list@latest': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null,
    ...
    describe('initialized with a different set of lists', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          ...
          lastInitializedDefaultListOfLists: ['https://unpkg.com/@thinkincoin-libs/default-token-list@latest'],
        })
        store.dispatch(updateVersion())
      })
